...
The current CI limitation is verified, not assumed: [Crow repository 10, pipeline 8](https://crow.corbet.ch/repos/10/pipeline/8), native step `21025`, recorded Rust 1.97.1 and Cargo 1.97.0, no `rustup` on the workflow PATH, and no target standard-library directories for either `aarch64-linux-android` or `aarch64-apple-ios`. That worker cannot repeat the latest mobile target checks without additional toolchain components. cmsg declares Rust 1.91 as its minimum, but this run does not verify that minimum. No tools were added for the inventory.
